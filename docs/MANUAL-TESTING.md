...
**Last updated:** Phase 8 (Demo data), 2026-09-22.
...
2. Docker is running: `docker compose up -d --wait` (MySQL + Mailpit), and `.env.local` exists
   (`composer app:secret` writes one with this machine's `APP_SECRET`).
...
Demo customers (every password is `password`; accounts are per shop, all of them in section 11):
`jan@example.test` has an account in **MyOil's Auto** (home address in Amsterdam, garage in Utrecht) and in
**MyOil's Industrie** (a company address), not in Agri.
...
| https://myoils-auto.shop.test/catalog | All 15 products, filters on the left, sort and grid/list switch at the top |
...
| **Pay now** | *Thank you for your order!*, the next order number (`AUTO-000012` after `composer demo:reset`), lines, addresses, totals; the cart badge is gone. After a moment (worker) the status becomes **Payment received** |
...
## 11. Demo data (Phase 8)

`composer demo:reset` builds everything below through the real services (pricing, stock, workflows,
payments, refunds), so the numbers add up everywhere. Demo orders send no emails.

| Per shop | What you get |
|---|---|
| Catalog | 15 products in 3-level category trees (e.g. *Brake fluids → Brake & clutch → DOT 4*, *Greases → Lithium greases → EP greases*, *Marine → Outboard engines → Outboard gear oils*), packs from 250 ml to 208 L drums, SDS and TDS links, some low or out of stock |
| Customers | 5 per shop, B2C and B2B (company and VAT number), each with a default billing and delivery address |
| Orders | About 10 orders over the last three weeks in **every** place: a customer's cart (draft), awaiting payment (one with a failed payment), paid, being prepared, shipped, delivered, cancelled before and after payment, refunded; with their payments, refunds and history |

| Shop | Customers (password `password`) |
|---|---|
| MyOil's Auto | `jan@example.test`, `sanne@example.test`, `inkoop@garagevisser.test` (Garage Visser B.V.), `wagenpark@taxinoord.test` (Taxi Centrale Noord), `mehmet@example.test` |
| MyOil's Industrie | `jan@example.test` (De Vries Techniek B.V.), `onderhoud@machinefabriekhoek.test`, `service@cnc-eindhoven.test`, `materieel@bouwgroen.test`, `lisa.smit@example.test` |
| MyOil's Agri & Marine | `info@loonbedrijfdeboer.test`, `havenmeester@hetanker.test`, `bas@boomverzorgingpeters.test`, `maatschap@veenstra.test`, `pieter.kuipers@example.test` |

| Try | You should see |
|---|---|
| Admin dashboard (manager, MyOil's Auto) | Revenue bars on several days, every status in *Orders by status*, latest orders |
| Admin → Orders, filter *Refunded*, open the order | Timeline *Awaiting payment → Payment received (System) → Being prepared → Shipped → Delivered → Refunded (Store Manager)* with notes; the payment shows the refunded amount |
| Log in at https://myoils-auto.shop.test/login as `sanne@example.test` | *Orders* lists her refunded order; her cart already holds 2 × Screenwash Winter |
| https://myoils-auto.shop.test/c/brake-fluids | Two DOT products through the sub-categories |
| Keep the worker running for an hour after the reset | The orders awaiting payment are cancelled by the expiry job (history: *Payment not received within 60 minutes.*) |
